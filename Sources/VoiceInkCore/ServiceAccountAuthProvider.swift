...
public actor ServiceAccountAuthProvider: GeminiAuthProvider {
  ...
  private func makeJWT(issuedAt: Date) throws -> String {
    ...
    let signature = try rsaSign(message: Data(signingInput.utf8), pemKey: credentials.privateKeyPEM)
  ...
  private func importPrivateKey(pem: String) throws -> SecKey {
    ...
    guard
      let secKey = SecKeyCreateWithData(pkcs1DER as CFData, attributes as CFDictionary, &cfError)
    else {
